    ...
    <div class="subheader flex-wrap">
        <div class="subheader-left pt-2 pb-2">
            <div class="subheader-breadcrumbs">
                <a href="{{route('DBbikes.index')}}" class="breadcrumbs-link">{{__('dashboard_forms.bikes')}}</a>
                <span class="breadcrumbs-separator-dot"></span>
                <a href="{{route('DBbikes.edit', $bike->id)}}" class="breadcrumbs-link">{{__('dashboard_forms.edit_bike')}} - {{$bike->id}}</a>
            </div>
        </div>
        ...
        <div class="card mb-1">
            ...
            <div class="card-body">
                {!! Form::model($bike, [
                        ...
                        'files' => true,
                        'id' => 'object-form',
                        'url' => route('DBbikes.update',[$bike->id, 'type' => request()->type, 'page' => request()->page,  'category' => request()->category, 'parent' => request()->parent ,'search' => request()->search, 'sort' => request()->get('sort'), 'direction' => request()->get('direction')]),
                      ]) !!}
